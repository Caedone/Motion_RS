...
for i in range(len(csv_file_list)):
    if first_file:
        ...
        final_row = df_name.iloc[-1].astype(float)  # Stores the final row of the first file
        subtraction = final_row - df_name.iloc[-2].astype(float)# Subtracts the last two rows
        ...
        w = 0
        while w < row_count:
            sum_row = (temp2 + subtraction).astype(float)  # Add final row + the subtacted rows
